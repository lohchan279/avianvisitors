(function () {
  ...
  function atlasStyle() {
    ...
    // Default comes from local-config.js, falling back to the stock
    // upstream value. Note: no apostrophes in this comment - the atlas
    // smoke test scans this source tracking quote characters and does not
    // skip comments, so a lone apostrophe hides the closing brace from it.
    // localCfg and this are both function declarations in the same scope,
    // so hoisting order does not matter.
    // typeof guard: the atlas smoke test evaluates this function on its
    // own, outside the closure, so localCfg is not in scope there. Absent
    // it, fall back to the stock upstream default.
    var cfg = (typeof localCfg === 'function')
      ? localCfg('atlasDefault', 'stamps') : 'stamps';
    var dflt = cfg === 'classic' ? 'classic' : 'stamps';
    return readLS(ATLAS_STYLE_KEY, dflt) === 'classic' ? 'classic' : 'stamps';
  }
